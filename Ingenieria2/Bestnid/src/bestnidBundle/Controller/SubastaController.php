...
class SubastaController extends Controller{
	...
	/**
     *
     * @Route("/busqueda/{paginaActual}/{msg}", name="busqueda")
     * @Template()
     */
    public function buscarAction($paginaActual = 0, $msg = 0)
    {		
		...
		$qb->select('s')
			...
			->where( $qb->expr()->like('s.titulo', $qb->expr()->literal('%'. $busqueda . '%')) )   
    ...
    public function showAction($id, $msg = 0)
    {
        ...
        return array(
			...
			'subastas_usuario' => $this->getSubastasDelUsuario($entity),
			'msg' => $msg,
	...
	private function getSubastasDelUsuario($Subasta){
		
		$repositorioSubastas= $this->getDoctrine()->getRepository('bestnidBundle:Subasta');
		
		$query = $repositorioSubastas->createQueryBuilder('p')
			->where('p.idUserSubastador = :User')
			->andWhere('p.id <> :id')
			->andWhere('p.estado = 0')
			->andWhere('p.fechaVencimiento > :fechaHoy')
			->setParameter('User', $Subasta->getIdUserSubastador()->getId())
			->setParameter('id', $Subasta->getId())
			->setParameter('fechaHoy', new \DateTime())
			->setMaxResults(4)
			->getQuery();
     ...
     *
     * @Route("/{id}/edit/{msg}", name="subasta_edit")
     * @Method("GET")
     * @Template()
     */
    ...
    public function deleteAction(Request $request, $id)
    {
        ...
        if ($form->isValid()) {
			...
			try{
				...
				return $this->redirect($this->generateUrl('subasta_edit',array('id' => $id,'msg' => 1)) );
				
			}
        }

        return $this->redirect($this->generateUrl('subastasUsuario',array('msg' => 2)) );
    }
